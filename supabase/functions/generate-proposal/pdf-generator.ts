...
export function generateProfessionalProposal(lead: any) {
  ...
  // Generate the PDF content (using existing PDF generation code)
...
/**
 * Generates the actual PDF content with the provided parameters
 */
function generatePDFContent(params: PDFParams): string {
  const {
    brandRed,
    companyName,
    contactName,
