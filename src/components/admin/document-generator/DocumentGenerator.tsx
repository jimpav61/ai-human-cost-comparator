...
export const DocumentGenerator = ({ lead }: DocumentGeneratorProps) => {
  ...
  const handleGenerateReport = async () => {
    try {
      ...
      console.log("---------- ADMIN REPORT GENERATION ATTEMPT ----------");
      console.log("Generating new report for lead:", lead.id);
      
      if (!lead.calculator_results || !lead.calculator_inputs) {
        throw new Error("Lead is missing calculator data required for report generation");
      }
      
      // Use the utility function to generate and save the report
      const success = await generateAndDownloadReport(lead);
      
      if (success) {
        toast({
          title: "Report Generated",
          description: "New report has been generated and downloaded successfully.",
        });
      } else {
      ...
      toast({
        ...
        description: error instanceof Error 
          ? error.message 
          : "Could not generate report. Missing calculator data.",
        variant: "destructive",
      });
    } finally {
      setIsGenerating(false);
      console.log("---------- ADMIN REPORT GENERATION ATTEMPT ENDED ----------");
    }
  };
  ...
  return (
    <div className="flex space-x-2">
      ...
      <Button
        ...
        disabled={isGenerating || !lead.calculator_results}
        variant="outline"
        size="sm"
        className="flex items-center"
      >
