...
export const TierComparison: React.FC<TierComparisonProps> = ({ 
  ...
  const renderTierCard = (tier: string) => {
    ...
    return (
      ...
      >
        ...
        {/* The key fix: Making sure the CollapsibleTrigger is INSIDE the Collapsible */}
        <Collapsible open={isExpanded} className="w-full">
          ...
          {/* CollapsibleTrigger must be a child of Collapsible */}
          <CollapsibleTrigger 
            asChild
            onClick={(e) => toggleTierExpanded(tier, e)}
            className="w-full"
          >
            <button className="text-xs text-brand-500 hover:text-brand-700 flex items-center justify-center w-full rounded-md py-2 hover:bg-gray-50">
              {isExpanded ? (
                <>
  ...
  return (
    <div className={isMobile ? "flex flex-col space-y-4" : "grid grid-cols-3 gap-4"}>
      {['starter', 'growth', 'premium'].map((tier) => renderTierCard(tier))}
    </div>
  );
};
